                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  for(int f_y=0; f_y<filterWidth; f_y++) {
    for(int f_x=0; f_x<filterWidth; f_x++) {
      int i_y = f_y + y- filterWidth/2;
      int i_x = f_x + x- filterWidth/2;
      i_y = min(max(i_y, 0), numRows-1);
      i_x = min(max(i_x, 0), numCols-1);
      if (i_y >=0 && i_y < numRows && i_x >=0 && i_x < numCols) {
        res += filter[f_y * filterWidth + f_x] * static_cast<float>(
          inputChannel[i_y * numCols + i_x]);
      }
    }
  }
  outputChannel[y * numCols + x] = res;
                        ...
                        const int filterWidth)
{
  ...
  int blockDim = 32;
  const dim3 blockSize(blockDim, blockDim, 1);

  //TODO:
  //Compute correct grid size (i.e., number of blocks per kernel launch)
  //from the image size and and block size.
  const dim3 gridSize(ceil(float(numCols)/blockDim), ceil(float(numRows)/blockDim), 1);
